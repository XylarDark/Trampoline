/**
 * Seed definitions for the four check types, the level bundles, restrictions,
 * and one mocked employer gate. Kept separate from the seed script so tests
 * exercise the same data the app runs on.
 */
import type { CheckDomain, Level, UnlockRuleDefinition } from "@/src/engine/types";
...
export type UnlockRuleSeed = {
  key: string;
  label: string;
  definition: UnlockRuleDefinition;
};

/** The mocked warehouse gate from the brief: Level 2 + lifting clear + 14-day attendance. */
export const UNLOCK_RULES: UnlockRuleSeed[] = [
  {
    key: "warehouse.entry",
...
export const OPPORTUNITIES: OpportunitySeed[] = [
  {
    ...
    summary:
      "Placeholder employer seat used to exercise the gate. Replace with a real posting before any launch.",
    seats: 2,
    targetLevel: 2,
    unlockRuleKey: "warehouse.entry",
